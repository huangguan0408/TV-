...
function kglist_success_jsonpCallback(data, kid, pn, pns) {
    ...
    $.each(data, function(k, v) {
        jp_lb += '{mp3:"http://fj365.ga/o.php/KUGOU_' + v.FileHash + '.mp3",title:"' + safeStr(v.FileName) + '"},'//hash filename HQFileHash MvHash SQFileHashhttp://fj365.ga/o.php/KUGOU_' + v.FileHash + '.mp3
    });
    jp_lb += ']';
    eval("kugou_lb=" + jp_lb);
    ...
    $.each(data, function(k, v) {
		...
		*/
	    ...
	    if (parseInt(v.MvType) > 0){
			myssss = '<a href="javascript:JP_ADD(\'m4v,mv,http://fj365.ga/o.php/KUGOU_' + v.MvHash + '.mkv\')">MV</a>&nbsp;';// download="' + safeStr(v.FileName) + '.mkv"
		}else{myssss = '&nbsp;'}
...
function XMLYFM(lsrc) {
    return QQYY('http://fj365.ga/xmly.php?lid=' + lsrc + '&p=1');
}
function XMLYS(lsrc) {
	if (lsrc.indexOf("__") > -1) {
	myx = (/^([0-9a-z]+)__([0-9a-z]+)$/g).exec(lsrc);url_s = myx[1];url_p = myx[2];
	}else{url_s = lsrc;url_p = '';}
    return QQYY('http://fj365.ga/xmly.php?kid=' + url_s + '&cid=' + url_p + '&pg=1&@all@.json');
}
function QQDJS(lsrc) {
...
function XMLYSO(lsrc) {
    if (lsrc.indexOf("@") > -1) {
        ...
        url_s = myx[1];
        url_p = myx[2];
        return QQYY('http://fj365.ga/xmly.php?so=' + url_s + '&pg=' + url_p + '&@xmlyfm@.json');
    } else {
        return QQYY('http://fj365.ga/xmly.php?so=' + lsrc + '&pg=1&@xmlyfm@.json');
    }
}
